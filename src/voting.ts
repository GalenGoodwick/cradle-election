// voting.ts — the heart of the Cradle.
//
// A cell's champion is decided by recused, outcome-grounded Borda count:
//   1. Each evaluator submits a best-first ranking of the OTHER candidates (never its own lens).
...
import type { Ballot, Memory, Standing, CellResult } from "./types.js";
...
export function tallyCell(candidates: Memory[], ballots: Ballot[]): CellResult {
  ...
  // Guard: a ballot must never rank its own lens (recusal is structural, not hoped-for).
